...
Importat issues:
    ...
    5. Target variable (imbalnace?)
      5.1 Cut-off point - can be chosen based on the distribution of the successes and failures
      in the sample. So for this dataset a good dataset may be around 11%. A 50% cut-off point
      will generally work poorly if the target is not equally balanced (50% - success, 50% - failure)
...
#prediction function
p_fun <- function(object, newdata){predict(object, newdata=newdata, type="prob")[,2]} # choose the prob of success to feed into explainer 
...
mean(test$class1)
opt_LR_ROC01$ROC01$Global$optimal.cutoff$cutoff
confusionMatrix(factor(ifelse(pred_LR>mean(test$class1),"yes","no")),test$class,positive ="yes") #average prop of successes
confusionMatrix(factor(ifelse(pred_LR>opt_LR_ROC01$ROC01$Global$optimal.cutoff$cutoff,"yes","no")), test$class, positive ="yes") # ROC1 optimal point
confusionMatrix(factor(ifelse(pred_LR>0.5,"yes","no")),test$class,positive ="yes") # classic 50%
...
We are starting with ML methods
...
THE PRED FUNCTION DOESN'T WORK -> OLD OBJECT
```{r message=FALSE, warning=FALSE}
...
EXPLAINER CHANGED BECAUSE XGB OBJECT GOT CORRUPTED
```{r}
library(rSAFE)
...
```
The dotted lines can be used as indicators of where to bin the variables
...
```
Using the bins to transform the train and test data (one-hot encoded) ->
...
```{r}
 ...
 # piv_dt_asp<-readRDS("piv_dt_asp.rds")
  ...
  plot(piv_lr_asp,piv_lr_reg_asp,piv_lr_reg_rs_asp,piv_rf_asp,piv_xgb_asp, bar_width=2)
